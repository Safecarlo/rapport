...
** Introduction
   ...
   Avant de commencer avec les benchmark NAS parallèle avec un peut
...
** Définition NAS Benchmarks parallèle
   ...
   parallèles qui est faible dans les benchmarks traditionnels.  Pour definir
   les NAS, c'est un outil développés et améliorer pour évaluer la perfomances
...
***  La version NPB3:
    ...
    et hybrides.et aussi un ensemble de benchmarks multi-zone un ensemble de
    benchmarks multi-zone tirant parti du modèle de programmation hybride MPI /
