...
*** Pull requests sur verificarlo
    ...
    anciens résultats n'étaient pas effacés (enfin on les enlevé manuellement au
    début puis nous avons vite arrêté). De plus il y avait 2 fichiers qui était
    créer à la configuration de verificarlo dans le répertoire qui n'était pas
    non plus nettoyé.
...
**** Métriques
     ...
     Donc le seuil de 6% est plus la à titre représentatif.
