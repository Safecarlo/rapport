...
** Différentes types de benchmark
  ...
  Pour les types de benchmark, on a garde la déscription qui sont détaillé dans le site
  officiel du NAS parallèle benchmark.
  
**** Cinq noyaux :
  
  - IS : C'est une methode de tri pour les entiers avec des accès de mémoire aléatoire.
...
** Résultats et discussion
  ...
  On a pris comme exemple le benchmark "BT" et de classe "S". Pour pouvoir exécuter le benchmark, il nous a
